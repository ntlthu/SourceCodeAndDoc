...
var form = document.getElementById("loremForm");
var amount = document.getElementById("amount");
var result = document.getElementById("loremText");

btnLorem.onclick = function (e) {
    e.preventDefault();
    var value = parseInt(amount.value);
    var random = Math.floor(Math.random() * text.length);

    if (value === "" || isNaN(value) || value < 0 || value > 9 || value === 1) {
        //random lorem paragraphs
        result.innerHTML = '<p class="result">'+text[random]+'</p>';
    } else {
        // show a number lorem paragraphs based on amount value
        var tempText = text.slice(0, value);
        var content = "";
        for (var i = 0; i < tempText.length; i++) {
          content += '<p class="result">'+tempText[i]+'</p>';
        }
