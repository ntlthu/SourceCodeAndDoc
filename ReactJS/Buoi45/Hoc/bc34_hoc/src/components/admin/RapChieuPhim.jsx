...
export default function RapChieuPhim() {
    ...
    return (
        <div className='text-left row'>
            <div className='col-3'>
                {dataRap?.map(item => {
                    return <h3><img onClick={() => layLichChieu(item.maHeThongRap)} width={50} src={item.logo} /></h3>
                })}
                ...
                {dataLichChieu?.map(item => {
                    return item.lstCumRap.map(itemCumRap => {
                        ...
                        return <div className='row'>
                            <div className='col-6'>
                                <img width={50} src={itemCumRap.hinhAnh} /> {itemCumRap.tenCumRap}
                            </div>
                            <div className='col-6'>
                                {itemCumRap.danhSachPhim.map(itemPhim => {
                                    return <h3> {itemPhim.tenPhim} <br />
                                        {itemPhim.lstLichChieuTheoPhim.map(itemLichChieu => {
                                            let dateNow = new Date();
                                            let ngayChieu = new Date(itemLichChieu.ngayChieuGioChieu);

                                            return <>{ngayChieu >= dateNow && moment(itemLichChieu.ngayChieuGioChieu).format("HH:mm") + "-"} </>
                                        })}
                                    </h3>
                                })}
                            </div>
